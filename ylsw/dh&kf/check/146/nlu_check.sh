...
# redis
if [ $(ps -ef | grep -v grep | grep redis | wc -l ) -eq 0 ];then
    # 启动redis
    echo "$(date +%H:%M:%S) 【ERROR】 redis 进程不存在，现在启动 redis " >> ${BASE_DIR}/check_nlu_$(date +%Y-%m-%d).log
    cd /app/redis/redis
    ./redis-server ./redis.conf
elif [ $(ps -ef | grep -v grep | grep redis | wc -l ) -gt 1 ];then
    echo "$(date +%H:%M:%S) 【ERROR】 redis 进程异常，重新启动 redis " >> ${BASE_DIR}/check_nlu_$(date +%Y-%m-%d).log
    ps -ef | grep redis | grep -v grep | awk '{print $2}' | xargs kill -9
    cd /app/redis/redis
    ./redis-server ./redis.conf
else
    echo "$(date +%H:%M:%S) 【INFO】 redis 进程 正常 " >> ${BASE_DIR}/check_nlu_$(date +%Y-%m-%d).log
fi
